...
# This script also generates the data reported in Table 1
...
# Race
table(ClinicalData$race_epic)

# Wound Etiology
table(ClinicalData$wound_type)

# Wound location
table(ClinicalData$woundloc)

# VAC use 
ClinicalData = ClinicalData %>% mutate(BinaryDressingType = if_else(dressingcat %in% c(2,3), "WoundVac", "NonWoundVac"))
table(ClinicalData$BinaryDressingType)

# Inflammation
table(ClinicalData$inflame)

# Pain
table(ClinicalData$woundcarepain)

# days since injury
table(ClinicalData$woundage)
...
# Table 2
#########
ClinicalData$study_id = sapply(ClinicalData$study_id, as.character)
AllData = FullBioData %>% left_join(ClinicalData, by="study_id")

# Wound care pain
#########################
tablePainDMM = AllData %>% select(DMMClusterAssign,woundcarepain) %>% filter(!is.na(DMMClusterAssign))
print(table(tablePainDMM))
chisq.test(table(tablePainDMM))

# Inflammation
tableinflameDMM = AllData %>% select(DMMClusterAssign,inflame) %>% filter(!is.na(DMMClusterAssign))
print(table(tableinflameDMM))
chisq.test(table(tableinflameDMM))
...
# Dressing type
AllData$BinaryDressingType =if_else(AllData$dressingcat %in% c(2,3), "WoundVAC","NonWoundVAC")
tableDressDMM = AllData %>% select(DMMClusterAssign,BinaryDressingType) %>% filter(!is.na(DMMClusterAssign))
chisq.test(table(tableDressDMM))

# Dressing type binary
BinaryDMMDressing = AllData %>% filter(wound_type %in% c(4,6)) %>% select(DMMClusterAssign,BinaryDressingType) %>% filter(!is.na(DMMClusterAssign))
chisq.test(table(BinaryDMMDressing))

# Wound type 
# Small # expected so simulated p 
DMM_type = AllData %>%  select(DMMClusterAssign,wound_type) 
chisq.test(table(DMM_type), simulate.p.value = T)
 
# Wound location          
# Small # expected so simulated p 
DMM_loc = AllData %>%  select(DMMClusterAssign,woundloc) 
chisq.test(table(DMM_loc), simulate.p.value = T)


# Genus abundances
FullDataRMNamicrobes = AllData %>% filter(!is.na(DMMClusterAssign))
median(FullDataRMNamicrobes$AnaerobicGenusAbundance,na.rm=T)*100
median(FullDataRMNamicrobes$CorynebacteriumAbundance,na.rm=T)*100
median(FullDataRMNamicrobes$PseudomonasAbundance,na.rm=T)*100
median(FullDataRMNamicrobes$StaphylococcusAbundance,na.rm=T)*100
median(FullDataRMNamicrobes$StreptococcusAbundance,na.rm=T)*100

mean(FullDataRMNamicrobes$AnaerobicGenusAbundance,na.rm=T)*100
mean(FullDataRMNamicrobes$CorynebacteriumAbundance,na.rm=T)*100
mean(FullDataRMNamicrobes$PseudomonasAbundance,na.rm=T)*100
mean(FullDataRMNamicrobes$StaphylococcusAbundance,na.rm=T)*100
mean(FullDataRMNamicrobes$StreptococcusAbundance,na.rm=T)*100

sd(FullDataRMNamicrobes$AnaerobicGenusAbundance,na.rm=T)*100
sd(FullDataRMNamicrobes$CorynebacteriumAbundance,na.rm=T)*100
sd(FullDataRMNamicrobes$PseudomonasAbundance,na.rm=T)*100
sd(FullDataRMNamicrobes$StaphylococcusAbundance,na.rm=T)*100
sd(FullDataRMNamicrobes$StreptococcusAbundance,na.rm=T)*100
